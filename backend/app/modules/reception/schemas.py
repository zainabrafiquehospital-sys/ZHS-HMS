...
class ReceptionRevenueOut(BaseModel):
    """A receptionist's own revenue — visits, medicine bills, and (Step
    ...
    `cleared_at` reports the effective window start — `max(last manual
    "Clear Revenue" action, now - 24h)` — so it is always a real, recent
    timestamp, never `None` and never an all-time cumulative view (see
    ReceptionService.get_own_revenue's own docstring for the full
    mechanism). Never anyone else's revenue — this response shape
    carries no `user_id` field at all, since the caller can only ever
    be asking about themselves (see ReceptionService.get_own_revenue:
    always `actor.id`, never a request parameter)."""
