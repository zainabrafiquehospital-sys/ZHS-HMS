...
class LabBillRepository(BaseRepository[LabBill]):
    ...
    async def cash_online_collected_for_creator(
        ...
        together) amounts actually *collected* via `lab_bill_payment`
        rows, for bills this user created — optionally only those created
        after `since`. See app/modules/visits/repository.py's identical
        `cash_online_collected_for_creator` for the full rationale."""
        conditions = [
            LabBill.deleted_at.is_(None),
            LabBill.created_by == user_id,
            LabBillPayment.deleted_at.is_(None),
        ]
        if since is not None:
            conditions.append(LabBill.created_at > since)
        is_cash = LabBillPayment.payment_method == PaymentMethod.CASH
        stmt = (
            select(
                func.sum(case((is_cash, LabBillPayment.amount), else_=None)),
                func.sum(case((~is_cash, LabBillPayment.amount), else_=None)),
            )
            .select_from(LabBillPayment)
            .join(LabBill, LabBill.id == LabBillPayment.lab_bill_id)
            .where(*conditions)
        )
